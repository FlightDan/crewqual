...
test.describe("admin operations workflow", () => {
  ...
  test("selected day lists each node pilot's six qualifications and edits in place", async ({
    ...
    await expect(pilot.getByRole("button", { name: /^编辑吴岚/ })).toHaveCount(6);
  ...
  test("qualification config validates, persists and leaves effective records unchanged", async ({
    ...
    await expect(page.getByText(/现有飞行员生效记录未被回写/)).toBeVisible();
    await page.reload();
  ...
  test("1440x1024 shows desktop calendar, tables and multi-column layouts", async ({ page }) => {
    ...
    await page.goto("/admin/upgrade-plans");
  ...
  test("calendar squadron filter supports selection and URL restoration", async ({ page }) => {
    await page.goto("/admin/calendar?date=2026-08-14");
