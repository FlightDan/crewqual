...
Final releases require `acceptance_scope=isolated` or `acceptance_scope=full`; they cannot use `local`. Isolated acceptance uses temporary PostgreSQL and MinIO services to run static checks, container bootstrap, candidate-image browser checks, supply-chain verification, real application database and gallery backups, isolated restoration, tamper rejection, and post-publish clean installation, upgrade, and rollback tests. It verifies the application's backup and recovery path, but it does not validate real AWS IAM, SSE-KMS, cross-region, or production disaster-recovery configuration. Full scope adds real S3 infrastructure controls and external disaster-recovery checks. The project Roadmap lists the corresponding unvalidated areas.

Because earlier stable releases do not contain arm64 images, `v1.0.6` uses the previous stable release for upgrade, rollback, and retry acceptance on amd64, and a previously accepted RC from the same commit for promotion acceptance on arm64. This covers arm64 clean installation, RC-to-stable upgrade, rollback, and retry. It does not establish arm64 stable-to-stable upgrade coverage; a later stable release can close that gap once an arm64 stable baseline exists.

RCs may use `acceptance_scope=local`; local scope runs static, bootstrap, supply-chain, and post-publish installation, upgrade, and rollback checks with temporary MinIO. It does not run the application backup-and-recovery gate and requires no S3, AWS, or external disaster-recovery configuration.

`acceptance_scope=isolated` and `acceptance_scope=full` are also available to RCs that need the corresponding gates. When using full scope, keep S3, AWS, and disaster-recovery configuration exclusively in the `release-sandbox` GitHub Environment; do not rely on repository-level secrets with the same names. Start formal releases through `crewqual-release-publish`, not by dispatching the workflow in the Actions UI. GitHub merges environment and repository scopes in the workflow secret context, so only the publisher's environment inventory check prevents accidental use of a repository secret with the same name. The publisher checks the full-scope secret names before signing or pushing a tag. The Actions workflow then aggregates value validation, OpenPGP key-to-allowlist matching, and isolation checks before building or pushing images, without printing secret values.
